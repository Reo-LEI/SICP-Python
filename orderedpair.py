...
def count_leave(item):
    if item == None: return 0
    elif not isinstance(item, tuple): return 1
    else: return count_leave(car(item))+count_leave(cdr(item))
...
def mapping(func, item):
    ...
    else: return cons(func(car(item)), mapping(func, cdr(item)))

def mapper(func, tree):
    # return a tree
    # 接受一个树，依次讲各个叶节点作为func的参数，返回树
    if tree == None: pass
    elif not isinstance(tree, tuple): return func(tree)
    else: return cons(mapper(func, car(tree)), mapper(func, cdr(tree)))
...
def mapping_n(func, *seqs):
    ...
    def seq_trans(seq):  # 将序列转化为元组
        def translator(s):
            if cdr(s) == None: return [car(s)]
            else: return [car(s)]+translator(cdr(s))
        return tuple(translator(seq))
        
    def _map(f, seq):
        if car(seq) == None: return None
        else: return cons(f(*seq_trans(car_n(seq))), _map(f, cdr_n(seq)))  # 用*解包元组调用f

    if len(seqs) > 1: seqs = lister(*seqs)  # 判断seqs为树或者多个列表并解包
    else: seqs = seqs[0]  # 若为树直接解包
...
def display(items):
    ...
    def order(item):
        ...
        elif not isinstance(item, tuple): return True
        else: return order(car(item))
    ...
    if order(items):
        ...
        def nested(item):
            if item == None: return False
            elif isinstance(cdr(item), tuple) and car(item) == None: return True
            else: return nested(car(item))
        ...
        def show(item):
            ...
            else:return show(car(item))+[show(cdr(item))]
...
##    print(reverse(listtest), '\n')
    r = deep_reverse(listtest)
    k = deep_reverse(p)
##    print(deep_reverse(listtest), '\n')
##    print(k, '\n')
    display(r)
    display(k)
    display(mapper(lambda x: x**2, listtest))
